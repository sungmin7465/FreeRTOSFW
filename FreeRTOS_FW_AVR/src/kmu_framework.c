...
static UBaseType_t fNumberOfTasks = ( UBaseType_t ) 0U;

/* fReadyTasks, fHideTasks에 관한 자세한 사항은 framework.h를 참조 하십시오. */
static FReadyTask fReadyTasks[configMAX_NUMBER_OF_TASKS];
static FHideParams fHideTasks[configMAX_NUMBER_OF_TASKS];
...
/* 일반적인 정렬 함수 입니다. 태스크를 '주기'에 따라 정렬합니다. (내림차순)*/
void fSort() {
    UBaseType_t c, d;
    FReadyTask t;
